...
Ziel: Du simulierst den Datenfluss zwischen **KIS, RIS, LIS, MWL, CT, PACS und Workstation** und erkennst, welche Informationen über **HL7** bzw. **DICOM** transportiert werden.

## Überblick (Workflow)

![Radiologie Workflow](workflow.png)

### Systeme & Abkürzungen (kurz)

**Systeme**
- **KIS** (Krankenhaus-Informationssystem): Patientenaufnahme und Stammdaten.
- **RIS** (Radiologie-Informationssystem): Radiologie-Auftrag, Terminierung, Status.
- **LIS** (Labor-Informationssystem): Laborwerte (z.B. Kreatinin).
- **MWL** (Modality Worklist): Termin-/Auftragsliste, die Geräte abrufen.
- **Modalität** (z.B. CT): Gerät, das Bilder erzeugt.
- **PACS** (Picture Archiving and Communication System): Bildarchiv (hier: Orthanc).
- **Workstation/Viewer**: Bilder anzeigen, Studien suchen, Retrieve.

**Protokolle / Nachrichten**
- **HL7** (Verwaltungs-/Patientendaten):
  - **ADT**: Aufnahme/Patientendaten
  - **ORU**: Labor-/Befund-Nachrichten
  - **ORM**: Auftrag/Order
- **DICOM** (Bilddaten & Bild-Workflow):
  - **C-FIND**: Suchen (z.B. Worklist/Studien)
  - **C-STORE**: Bilder senden
  - **C-MOVE**: Bilder anfordern (Retrieve)

## Vorbereitung

- App öffnen: https://orthan.bohn-teaching.org
